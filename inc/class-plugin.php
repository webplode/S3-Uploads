...
class Plugin {
	...
	public function setup() : void {
		...
		add_filter( 'wp_handle_upload', [ $this, 'log_upload_errors' ], 5, 2 );
		add_filter( 'wp_handle_upload', [ $this, 'maybe_keep_local_copy_for_fonts' ], 10, 2 );
		
		// Generate .htaccess rules once (not on every request).
		add_action( 'init', [ $this, 'maybe_generate_htaccess_rules' ], 20 );
	...
	public function enable_webp_support( array $data, string $file, string $filename, ?array $mimes ) : array {
		...
		// Prefilter uses names like photo.jpg.webp; ensure MIME is still accepted.
		if ( preg_match( '/\.webp$/i', $filename ) && is_readable( $file ) ) {
			$detected = wp_get_image_mime( $file );
			if ( 'image/webp' === $detected ) {
				$data['ext']             = 'webp';
				$data['type']            = 'image/webp';
	...
	/**
	 * Generate .htaccess rules once when missing or outdated.
	 */
	public function maybe_generate_htaccess_rules() : void {
		if ( ! $this->is_htaccess_rules_enabled() ) {
			return;
		}
		...
		$marker              = '# Generated by S3-Uploads WebP Plugin';

		if ( file_exists( $htaccess_file ) ) {
			$content = file_get_contents( $htaccess_file );
			if ( is_string( $content ) && strpos( $content, $marker ) !== false ) {
				return;
			}
		}
	...
	/**
	 * Whether WebP .htaccess redirect rules should be written.
	 *
	 * @return bool
	 */
	protected function is_htaccess_rules_enabled() : bool {
		if ( defined( 'S3_UPLOADS_DISABLE_HTACCESS' ) && S3_UPLOADS_DISABLE_HTACCESS ) {
			return false;
		}

		return (bool) apply_filters( 's3_uploads_enable_htaccess_rules', true );
	}
	...
	public function generate_htaccess_rules() {
		...
		$htaccess_content .= "# Redirect all other files to CDN\n";
	...
	public function convert_original_to_webp_prefilter( array $file ) {
		if ( defined( 'S3_UPLOADS_DISABLE_WEBP_PREFILTER' ) && S3_UPLOADS_DISABLE_WEBP_PREFILTER ) {
			return $file;
		}

		if ( ! apply_filters( 's3_uploads_webp_prefilter_enabled', true, $file ) ) {
			return $file;
		}
	...
	/**
	 * Log failed uploads (helps when the UI only shows a generic AJAX error).
	 *
	 * @param array  $upload
	 * @param string $context
	 * @return array
	 */
	public function log_upload_errors( array $upload, string $context ) : array {
		if ( ! empty( $upload['error'] ) ) {
			error_log( '[S3-Uploads] wp_handle_upload error (' . $context . '): ' . $upload['error'] );
		}
		return $upload;
	}
