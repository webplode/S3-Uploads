...
class Plugin {
	...
	public function generate_htaccess_rules() {
		...
		if ( $result !== false ) {
			error_log( '[S3-Uploads WebP] Generated .htaccess rules at: ' . $htaccess_file );
		} else {
			error_log( '[S3-Uploads WebP] Failed to write .htaccess rules to: ' . $htaccess_file );
		}
	}
	...
	public function remove_htaccess_rules() {
		...
		if ( file_exists( $htaccess_file ) ) {
			...
			if ( strpos( $content, '# Generated by S3-Uploads WebP Plugin' ) !== false ) {
				...
				if ( $result ) {
					error_log( '[S3-Uploads WebP] Removed .htaccess rules from: ' . $htaccess_file );
				} else {
					error_log( '[S3-Uploads WebP] Failed to remove .htaccess rules from: ' . $htaccess_file );
				}
			} else {
				error_log( '[S3-Uploads WebP] .htaccess file exists but was not generated by S3-Uploads WebP Plugin, leaving it untouched' );
			}
		}
	}
}
